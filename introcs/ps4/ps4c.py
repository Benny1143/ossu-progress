...
class SubMessage(object):
    ...
    def build_transpose_dict(self, vowels_permutation):
        ...
        '''
        # VOWELS_LOWER
        dic = {letter: letter for letter in string.ascii_letters}
        for i in range(len(vowels_permutation)):
            vowel = vowels_permutation[i]
            dic[VOWELS_LOWER[i]] = vowel
            dic[VOWELS_UPPER[i]] = vowel.upper()
        return dic
...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        '''
        # get_permutations
        permu = get_permutations(VOWELS_LOWER)
        decrypt = ""
        bestrealwords = 0
        for permutation in permu:
            words = self.apply_transpose(
                self.build_transpose_dict(permutation))
            wordcount = 0
            for word in words.split():
                if is_word(self.valid_words, word):
                    wordcount += 1
            if wordcount > bestrealwords:
                bestrealwords = wordcount
                decrypt = words
        return decrypt
...
if __name__ == '__main__':
    ...
    message = SubMessage("Fishing Salmon")
    permutation = "oiuae"
    enc_dict = message.build_transpose_dict(permutation)
    print("Original message:", message.get_message_text(),
          "Permutation:", permutation)
    print("Expected encryption:", "Fushung Solman")
    print("Actual encryption:", message.apply_transpose(enc_dict))
    enc_message = EncryptedSubMessage(message.apply_transpose(enc_dict))
    print("Decrypted message:", enc_message.decrypt_message())
